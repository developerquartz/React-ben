...
import { exportAdminTransactions } from "../../../../../store/actions";
...
const ExportCsv = (props) => {
  ...
  const Data = useSelector((s) => s?.Export?.exportAdminTransactions?.data);
      ...
      Data.map((k) => {
        return {
          ...
          createdAt: k["createdAt"]
            ? moment(k["createdAt"]).format("YYYY-MM-DD, hh:mm A")
            : "",
            customerDetails: k["customerDetails"]? k["customerDetails"]?.name : ""
        };
      });
  ...
  const fetchData = () => {
    ...
    dispatch(exportAdminTransactions(body));
  };
  ...
  useEffect(() => {
    let column = columns && columns.length > 0 &&
      columns.map((key) => {
          let no = ["_id", "action"];
